...
To generate more data computationally, I will use Option 1: scale up to 500+1,000+ items using automation. I plan to use web scraping of digital news outlets to collect articles covering NFL player statements, along with pattern matching to extract key features such as Media Tone, Event Type, Platform, and Post Status. With pattern matching, I would have to create an algorithm where if certain words were used, then the "interpretative" variables like the Media Tone and Event Type could be categorized as so. For example, "criticized" equiv to "Controversy." For the Media Tone, it does utilize the use of language to address the tone of the article, but also the content of the coverage plays a role. To explain further, if the event is a "serious" manner like fighting another player online, using inappropriate or offensive language, etc., the outlet would have to address those "negative" situations which affects the tone in which the media outlet is setting. The limitations may include the risk of missing nuances, sarcasm, or mixed framing, as well as web scraping not fully capturing all relevant sources. When I think about interpretation, it is difficult to have technology understand every parts of the English languages that shouldn't be taken at face-value. So, there is a risk of misclassification with the interpretative variables, Media Tone and Event Type. 
